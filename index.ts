...
// ====== RUN QUEST IN CHILD PROCESS (ULTRA FIX) ======
async function runQuestInChildProcess(token: string): Promise<string> {
    return new Promise((resolve, reject) => {
        // Tạo file .env tạm thời với token
        const envPath = path.join(__dirname, '.env.quest');
        fs.writeFileSync(envPath, `TOKEN=${token}\n`);
        
        // Tạo file script wrapper để chạy quest
        const wrapperPath = path.join(__dirname, 'quest-wrapper.js');
        const wrapperContent = `
            ...
            // Timeout sau 30 phút
            setTimeout(() => {
                console.log('⏰ Quest timeout, exiting...');
                process.exit(0);
            }, 30 * 60 * 1000);
        `;
        ...
        child.stdout.on('data', (data) => {
            ...
            output += str;
            console.log('[Quest]', str);
        });
        
        child.stderr.on('data', (data) => {
            const str = data.toString();
            errorOutput += str;
            console.error('[Quest Error]', str);
        });
        
        // Không dùng 'close' mà dùng 'exit' để bắt cả crash
        child.on('exit', (code, signal) => {
            // Xóa file tạm
            try { fs.unlinkSync(envPath); } catch (e) {}
            try { fs.unlinkSync(wrapperPath); } catch (e) {}
            
            // Dù code thế nào, quest đã chạy xong
            if (code === 0 || code === null || signal === 'SIGTERM' || signal === 'SIGINT') {
                resolve(output || 'Quest completed (may have been interrupted)');
            } else {
                // Nếu có lỗi nhưng output vẫn có, vẫn coi là thành công 1 phần
                if (output.length > 0) {
                    resolve(output);
                } else {
                    reject(new Error(`Quest failed with code ${code}: ${errorOutput || 'Unknown error'}`));
                }
            }
        });
...
// Giữ bot chạy kể cả khi có lỗi
process.on('exit', (code) => {
    console.log(`⚠️ Process exit with code ${code}`);
});

// Bắt SIGTERM (Render gửi khi restart)
process.on('SIGTERM', () => {
    console.log('🔄 Received SIGTERM, keeping bot alive...');
});
